...
class EventQuestion(models.Model):
    ...
    def action_view_question_answers(self):
        ...
        if event_id := self._context.get('search_default_event_id'):
        ...
        action['domain'] = [('event_id.question_ids', 'in', self.id)]

        if self.question_type == 'simple_choice':
            action['views'] = [(False, 'graph'), (False, 'pivot'), (False, 'list')]
            # Fetch attendee answers for which the value_answer_id is still linked to the question.
            action['domain'] = AND([action['domain'], [('value_answer_id.question_id', 'in', self.id)]])
    ...
    def action_event_view(self):
        ...
        action['domain'] = [('question_ids', 'in', self.id)]
        return action
