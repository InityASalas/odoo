...
class TestHrContracts(TestHrCommon):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.contracts = cls.env['hr.version'].with_context(tracking_disable=True)
    ...
    def test_set_employee_contract_create(self):
        ...
        self.assertEqual(self.employee.version_id, contract)
    ...
    def test_current_contract_stage_change(self):
        ...
        self.assertEqual(self.employee.version_id, draft_contract)
    ...
    def test_copy_employee_contract_create(self):
        ...
        self.assertNotEqual(duplicate_employee.version_id, contract)
    ...
    def test_check_multi_company_contract_expiration(self):
        ...
        with freeze_time('2015-11-01'):
            self.env['hr.version'].update_state()

            mail_activity = self.env['mail.activity'].search([('res_id', '=', contract_1.id), ('res_model', '=', 'hr.version')])
            ...
            mail_activity2 = self.env['mail.activity'].search([('res_id', '=', contract_2.id), ('res_model', '=', 'hr.version')])
