...
# |   1 |  Certificate |    Odoo  |  |     1 |  Certificate |       20% |                               3 |
# |     |              |          |  |     2 |  Certificate |       50% |                               2 |
# |     |              |          |  |     3 |  Certificate |       70% |                               1 |
# |     |              |          |  |     4 |  Certificate |      100% |                               0 |
# |-------------------------------|  |----------------------------------|---------------------------------|
# |   2 |    Languages |  Arabic  |  |     5 |    Languages |        A1 |                               5 |
# |   3 |    Languages | English  |  |     6 |    Languages |        A2 |                               4 |
# |   4 |    Languages |  French  |  |     7 |    Languages |        B1 |                               3 |
# |     |              |          |  |     8 |    Languages |        B2 |                               2 |
# |     |              |          |  |     9 |    Languages |        C1 |                               1 |
# |     |              |          |  |    10 |    Languages |        C2 |                               0 |
# |-------------------------------|  |----------------------------------|---------------------------------|
...
# |    2 |  Certificate |    Odoo |     20% |        True  |     24-01-01 |    24-04-01 |
# |    3 |    Languages | English |     A2  |       False  |     24-01-01 |           - |
# |    4 |    Languages |  Arabic |     A2  |       False  |     24-02-01 |           - |
# |    4 |    Languages |  Arabic |     A1  |       False  |     24-01-01 |    24-01-31 | <- not present in current_employee_skill
# |-------------------------------------------------------------------------------------|
        ...
        cls.line1, cls.line2, cls.line3, cls.line4, cls.line5 = cls.env['hr.employee.skill'].create([
            {
                ...
                'skill_level_id': cls.certification.skill_level_ids[2].id,
                'employee_id': cls.employee.id,
                'valid_from': datetime.date(2024, 3, 2),
            }, {
                'skill_type_id': cls.certification.id,
                'skill_id': cls.certification.skill_ids[0].id,
                'skill_level_id': cls.certification.skill_level_ids[3].id,
                'employee_id': cls.employee.id,
                ...
                'skill_level_id': cls.language.skill_level_ids[4].id,
                'employee_id': cls.employee.id,
                'valid_from': datetime.date(2024, 1, 1),
            }, {
                'skill_type_id': cls.language.id,
                ...
                'skill_type_id': cls.language.id,
                'skill_id': cls.language.skill_ids[0].id,
                'skill_level_id': cls.language.skill_level_ids[5].id,
                'employee_id': cls.employee.id,
                'valid_from': datetime.date(2024, 1, 1),
    ...
    def test_add_english_b1(self):
        ...
        with employee_form.current_employee_skill_ids.new() as employee_skill_form:
            ...
            employee_skill_form.skill_level_id = self.language.skill_level_ids[3]
    ...
    def test_edit_english_a2_to_english_b1(self):
        employee_form = Form(self.employee)
        previous_employee_skills = self.employee.employee_skill_ids
        # Skill are order by skill type name, skill name, skill level (will break if the order defined in the view changed)
        # Employee Skill Order, Odoo 50%, Odoo 20%, Arabic A2, English A2
    ...
    def test_edit_odoo_20_to_french_a1(self):
        employee_form = Form(self.employee)
        # Skill are order by skill type name, skill name, skill level (will break if the order defined in the view changed)
        # Employee Skill Order, Odoo 50%, Odoo 20%, Arabic A2, English A2
        previous_employee_skills = self.employee.employee_skill_ids
        index = self.employee.current_employee_skill_ids.ids.index(self.line2.id)
        with employee_form.current_employee_skill_ids.edit(index) as employee_skill_form:
            employee_skill_form.skill_type_id = self.language
            employee_skill_form.skill_id = self.language.skill_ids[2]
            employee_skill_form.skill_level_id = self.language.skill_level_ids[5]

        employee = employee_form.save()
        new_employee_skill = employee.employee_skill_ids - previous_employee_skills
        delete_one = previous_employee_skills - employee.employee_skill_ids
        self.assertEqual(len(employee.employee_skill_ids.ids), 5)
    ...
    def test_add_odoo_50_from_1_jan_to_infinite(self):
        ...
        with employee_form.current_employee_skill_ids.new() as employee_skill_form:
            employee_skill_form.skill_type_id = self.certification
            employee_skill_form.skill_id = self.certification.skill_ids[0]
            employee_skill_form.skill_level_id = self.certification.skill_level_ids[2]  # so same as odoo 50%
    ...
    def test_same_certification_with_different_levels_but_same_dates_can_coexist(self):
        ...
        self.assertEqual(len(self.employee.current_employee_skill_ids), 5, "The new certification should be added")
    ...
    def test_duplicate_certifications_in_the_past_are_not_created(self):
        ...
        self.assertTrue(new_skill)
        self.assertEqual(len(self.employee.employee_skill_ids), 6)
        self.assertEqual(len(self.employee.current_employee_skill_ids), 5)
    ...
    def test_rpc_call_editing_range_date_regular_skill(self):
        ...
        french_a1, french_a2, french_a3 = self.env['hr.employee.skill'].create([
            {
                'skill_type_id': self.language.id,
                'skill_id': self.language.skill_ids[2].id,
                'skill_level_id': self.language.skill_level_ids[5].id,
                ...
                'skill_type_id': self.language.id,
                'skill_id': self.language.skill_ids[2].id,
                'skill_level_id': self.language.skill_level_ids[4].id,
                'employee_id': self.employee.id,
                ...
                'skill_type_id': self.language.id,
                'skill_id': self.language.skill_ids[2].id,
                'skill_level_id': self.language.skill_level_ids[3].id,
                'employee_id': self.employee.id,
