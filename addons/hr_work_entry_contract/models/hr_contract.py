    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        ...
        for contract in self:
            ...
            for interval in real_worked_leaves:
                work_entry_type = contract._get_interval_leave_work_entry_type(interval, worked_leaves, bypassing_work_entry_type_codes)
                # All benefits generated here are using datetimes converted from the employee's timezone
                contract_vals += [dict([
                    ('name', "%s: %s" % (work_entry_type.name, employee.name)),
                    ('date_start', interval[0].astimezone(pytz.utc).replace(tzinfo=None)),
                    ...
                    ('company_id', contract.company_id.id),
                    ('state', 'draft'),
                ] + contract._get_more_vals_leave_interval(interval, worked_leaves))]
