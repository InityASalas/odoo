...
export class TableAlignPlugin extends Plugin {
    ...
    get currentVerticalAlign() {
        const traversedCells = this.dependencies.selection
            .getTraversedNodes()
            .map((node) => closestElement(node, "td, th"))
            .filter(Boolean);

        if (!traversedCells.length) {
            return "";
        }

        const verticalAlign = traversedCells[0].style.verticalAlign;
        return verticalAlign &&
            traversedCells.every((cell) => cell.style.verticalAlign === verticalAlign)
            ? verticalAlign
            : "";
    }
