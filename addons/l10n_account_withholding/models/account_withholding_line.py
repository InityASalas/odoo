...
class AccountWithholdingLine(models.AbstractModel):
    ...
    def _compute_original_amounts(self):
        ...
        for line in self:
            ...
            if not source_curr:
                rate = 1.0
                base_amount = line.base_amount  # line._get_default_base_amount_when_no_source_currency()
                if line.tax_id:
    ...
    def _constrains_account_id(self):
        """ The account on the line cannot be one deemed as liquidity account, otherwise it will cause issue. """
        for line in self:
    ...
    def _get_grouping_key(self):
        ...
        self.ensure_one()
        # Note, keep this aligned with _prepare_withholding_lines_commands
        return frozendict({
            'name': self.name,
    ...
    def _prepare_withholding_lines_commands(self, base_lines, company):
        ...
        def grouping_function(base_line, tax_data):
            account = company.withholding_tax_base_account_id or base_line['account_id']
            tax = tax_data['tax']
            # Note: keep this aligned with _get_grouping_key
            return {
                'name': base_line.get('manual_tax_line_name', tax.name),
