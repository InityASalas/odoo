...
class TestEfakturCoretax(AccountTestInvoicingCommon):
    ...
    def test_efaktur_download_separate(self):
        ...
        empty_document = self.env['l10n_id_efaktur_coretax.document']
        # Create 2 invoices
        invoices = self.env["account.move"].create([{
            "move_type": "out_invoice",
            "partner_id": self.partner_a.id,
        ...
        self.assertNotEqual(invoices[0].l10n_id_coretax_document, empty_document)
        self.assertNotEqual(invoices[1].l10n_id_coretax_document, empty_document)
