...
describe("Remove attachments", () => {
    ...
    test("should remove file from html editor if removed from attachment list", async () => {
        ...
        await animationFrame();
        expect("[name='attachment_ids'] a:contains('file.txt')").toHaveCount(1);
        expect(".odoo-editor-editable .o_file_box:has(a:contains('file.txt'))").toHaveCount(1);
        await contains("[name='attachment_ids'] button:has(i.fa-times)").click();
        await animationFrame();
        expect("[name='attachment_ids'] a:contains('file.txt')").toHaveCount(0);
        expect(".odoo-editor-editable .o_file_box:has(a:contains('file.txt'))").toHaveCount(0);
    });
    ...
    test("should remove image from html editor if removed from attachment list", async () => {
        ...
        await animationFrame();
        expect("[name='attachment_ids'] a:contains('test.jpg')").toHaveCount(1);
        expect(".odoo-editor-editable img[data-attachment-id='1']").toHaveCount(1);
        await contains("[name='attachment_ids'] button:has(i.fa-times)").click();
        expect("[name='attachment_ids'] a:contains('file.txt')").toHaveCount(0);
        expect(".odoo-editor-editable img[data-attachment-id='1']").toHaveCount(0);
    });
});
