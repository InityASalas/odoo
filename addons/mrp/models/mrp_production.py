...
class MrpProduction(models.Model):
    ...
    def _set_qty_producing(self, pick_manual_consumption_moves=True):
        ...
        for move in (
            ...
            | self.move_finished_ids.filtered(lambda m: m.product_id != self.product_id)
            | self.move_finished_ids.filtered(lambda m: m.product_id == self.product_id and m.product_id.tracking == 'serial')
        ):
            ...
            if (not move.manual_consumption or pick_manual_consumption_moves) and (move.product_id != self.product_id or move.product_id.tracking != 'serial'):
                move.picked = True
