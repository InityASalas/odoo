...
test("test drag and drop a task to schedule in calendar view in month scale", async () => {
    ...
    expectedDate = serializeDate(luxon.DateTime.fromISO(dateCell.dataset.date));
...
// TODO: how to simulate a pointer enter / leave ?
test.skip("test drag and drop a task to schedule in calendar view in week scale", async () => {
    let expectedDate = null;
    onRpc("project.task", "plan_task_in_calendar", ({ args }) => {
        const [taskIds, vals] = args;
        expect(taskIds).toEqual([10])
    ...
    const { drop, moveTo } = await contains(".o_task_to_plan_draggable:first").drag();
    const dateCell = queryFirst(".fc-day.fc-day-today.fc-daygrid-day");
    const { x } = dateCell.getBoundingClientRect();
    const slotCell = queryFirst(".fc-timegrid-slot.fc-timegrid-slot-lane");
    const { y } = slotCell.getBoundingClientRect();
    expectedDate = serializeDate(luxon.DateTime.fromISO(dateCell.dataset.date));
    await moveTo({ position: { x, y }});
    expect(".o_task_event_to_plan_container").toBeVisible();
    expect(".o_task_event_to_plan_container").toHaveClass("o-highlight");
    await drop();
    expect.verifySteps(['plan task']);
    expect(".o_task_to_plan_draggable").toHaveCount(1);
    expect(".o_task_to_plan_draggable").toHaveText("Task-11");
});
