...
class SaleOrder(models.Model):
    ...
    def _compute_show_project_and_task_button(self):
        ...
        for order in self:
            ...
            order.show_task_button = state and (any(order.order_line.mapped('project_id')) or order.tasks_count)
        ...
        if project or task:
            ...
            if project and not project.sale_line_id:
                ...
                if not project.reinvoiced_sale_order_id:
                    project.reinvoiced_sale_order_id = next(iter([created_records]), False)
