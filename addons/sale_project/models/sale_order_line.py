...
class SaleOrderLine(models.Model):
    ...
    def check_product_service_policy(self):
    ...
    def _create_task_template(self, project, template):
        ...
        template_count = sum(
                ...
                and sol.product_id.service_policy in sol.check_product_service_policy()
            ) or 1
        custom_name_prefix = False if project.sale_line_id else (self.order_id.name or '') + ' - '
        task = template.with_context(
                copy_from_template=True,
                custom_name_prefix=custom_name_prefix,
                ).sudo().copy({
                'allocated_hours': (template.allocated_hours or self._convert_qty_company_hours(self.company_id)) * template_count,
                'partner_id': self.order_id.partner_id.id,
                'project_id': project.id,
                'sale_line_id': self.id,
                'sale_order_id': self.order_id.id,
    ...
    def _timesheet_create_task(self, project):
        ...
        if template := self.product_id.task_template_id:
            task = self._create_task_template(project, template)
        else:
