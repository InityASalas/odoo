...
class StockPackage(models.Model):
    ...
    def _compute_json_popover(self):
        for package in self:
            ...
            package.json_popover = json.dumps({
                ...
                'msg': self.env._("This package is currently set to be sent in %s.", location_names),
                'color': 'text-warning',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get('name'):
                vals['name'] = self.env['ir.sequence'].next_by_code('stock.package')
            ...
            if package_type.identification_method == 'auto':
                for package in self:
                    if package.package_type_id != package_type:
                        package.name = package_type.sequence_id.next_by_id()
    ...
    def _get_all_package_dest_ids(self):
        """ Gets all parent packages recursively.
            Since we can only have a single _parent field on the model, we need to do this manually.
