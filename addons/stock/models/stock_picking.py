...
class StockPicking(models.Model):
    ...
    def _compute_packages_count(self):
        ...
        packages = self.env['stock.package'].search([('picking_ids', 'in', self.ids)])
        for pack in packages:
            for picking in pack.picking_ids:
                packages_by_pick[picking] += 1

        histories_by_pick = self.env['stock.move.line']._read_group([('picking_id', 'in', self.ids), ('picking_id.state', '=', 'done')], ['picking_id'], ['package_history_id:count_distinct'])
