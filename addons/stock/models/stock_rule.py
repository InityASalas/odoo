...
class StockRule(models.Model):
    ...
    picking_type_id = fields.Many2one(
        ...
        domain="[('id', 'in', picking_type_code_domain_ids)]")
    picking_type_code_domain_ids = fields.Many2many(
        'stock.picking.type', compute='_compute_picking_type_code_domain_ids')
    ...
    @api.depends('action')
    def _compute_picking_type_code_domain_ids(self):
        for rule in self:
            domain = rule._get_picking_type_code_domain().get(rule.action, [])
            rule.picking_type_code_domain_ids = rule.env['stock.picking.type'].search(domain).ids
