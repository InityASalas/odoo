...
class TestCompute(common.TransactionCase):
    ...
    def test_03_server_action_code_history_wizard(self):
        def get_history(action):
            return self.env["ir.actions.server.history"].with_context(active_test=False).search([("action_id", "=", action.id)])

        def assert_history(action, expected):
            history = get_history(action)
            self.assertRecordValues(history, expected)
            self.assertRecordValues(history, [{"active": index == 0} for index in range(len(expected))])
        ...
        with freeze_time("2025-05-12 09:30:00"):
            with Form(self.env['server.action.history.wizard'].with_context(action_id=action.id)) as wizard_form:
