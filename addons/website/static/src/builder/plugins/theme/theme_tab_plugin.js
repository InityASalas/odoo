...
export class ThemeTabPlugin extends Plugin {
    ...
    resources = {
        ...
        builder_actions: {
            customizeGray: new CustomizeGrayAction(this),
            changeColorPalette: new ChangeColorPaletteAction(this),
        },
        theme_options: [
            withSequence(
...
class CustomizeGrayAction extends BuilderAction {
    setup() {
        this.customHistory = this.dependencies.customizeWebsite.withCustomHistory;
    }
    getValue({ params: { mainParam: grayParamName } }) {
        return this.plugin.grayParams[grayParamName];
    }
    async apply({ params: { mainParam: grayParamName }, value }) {
        ...
        this.plugin.grayParams[grayParamName] = parseInt(value);
        for (let i = 1; i < 10; i++) {
            const key = (100 * i).toString();
            this.plugin.grays[key] = this.plugin.buildGray(key);
        }

        // Save all computed (JS side) grays in database
        await this.dependencies.customizeWebsite.customizeWebsiteColors(this.plugin.grays, {
            colorType: "gray",
        });
    }
}
