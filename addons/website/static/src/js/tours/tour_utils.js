...
export function insertSnippet(snippet, { position = "bottom", ignoreLoading = false } = {}) {
    ...
    const snippetSelector = snippet.id ? `[data-snippet="${snippet.id}"]` : `[data-snippet^="${snippet.customID}"]`;
    const snippetIDSelector = `[data-snippet-id='${snippet.id}']`;
    if (snippet.groupName) {
        ...
        },
        {
            ...
            trigger: `:iframe .o_snippet_preview_wrap > ${snippetSelector}:not(.d-none), :iframe .o_snippet_preview_wrap${snippetIDSelector}:not(.d-none)`,
            noPrepend: true,
            tooltipPosition: "top",
            run: "click",
        });
