...
class ProductProduct(models.Model):
    ...
    def _prepare_gmc_items(self):
        ...
        shipping_info = records[:1]._prepare_gmc_shipping_info(
            delivery_methods_sudo, all_countries,
        )
        ...
        return {
            product: {
                ...
                **product._prepare_price_info(),
                **product._prepare_gmc_stock_info(),
                **product._prepare_additional_info(),
                **shipping_info,
            }
            for product in records if product._is_variant_possible()
        }
    ...
    def _prepare_price_info(self, format='google'):
        ...
        if combination_info['has_discounted_price']:
            ...
            if start_date and end_date:
                price_info['sale_price_effective_date'] = utils.format_sale_price_effective_date(
                    start_date, end_date, format=format,
                )
        ...
        if base_unit_name and self.base_unit_count:
            ...
            match = (
                const.META_BASE_MEASURE.match(unit_name)
                if format == 'meta'
                else const.GMC_BASE_MEASURE.match(unit_name)
            )
    ...
    def _prepare_meta_items(self):
        ...
        shipping_info = records[:1]._prepare_meta_shipping_info(
            delivery_methods_sudo, all_countries,
        )

        return {
            product: {
                'gtin': product.barcode if product.barcode else '',
                'rich_text_description': html_escape(str(product.description)),
                **product._prepare_common_feed_items(format_product_link),
                **product._prepare_feed_image_links(base_url),
    ...
    def _prepare_meta_shipping_info(self, delivery_methods_sudo, countries):
        ...
        for country, delivery in list(best_delivery_by_country.items())[:100]:
            price = (
                0.0 if delivery.get('free_over_threshold') and delivery['price'] == 0.0
                else delivery['price']
            )
            price_str = f"{price} {delivery['currency'].name}"
