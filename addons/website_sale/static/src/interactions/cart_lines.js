...
export class CartLines extends Interaction {
    ...
    async incOrDecQuantity(ev, currentTargetEl) {
        ...
        const minQuantity = parseFloat(input.dataset.min || 0);
        ...
        const newQuantity = currentTargetEl.querySelector('i').classList.contains('fa-minus')
            ? Math.max(oldQuantity - 1, minQuantity) : Math.min(oldQuantity + 1, maxQuantity);
...
registry
    ...
    .add('website_sale.cart_lines', CartLines);
