...
class ProductsItemOptionPlugin extends Plugin {
    ...
    resources = {
        ...
        builder_actions: {
            setItemSize: new SetItemSizeAction(this),
            changeSequence: new ChangeSequenceAction(this),
            setRibbon: new SetRibbonAction(this),
            createRibbon: new CreateRibbonAction(this),
            modifyRibbon: new ModifyRibbonAction(this),
            deleteRibbon: new DeleteRibbonAction(this),
        },
    };
...
class SetItemSizeAction extends BuilderAction {
    ...
    isApplied({ editingElement, value: [i, j] }) {
        if (
            ...
            this.plugin.itemSize.x = j + 1;
            this.plugin.itemSize.y = i + 1;
            return true;
        }
        return false;
    }
    apply({ editingElement, value: [i, j] }) {
        ...
        this.plugin.productTemplateID = parseInt(
            editingElement
                .querySelector('[data-oe-model="product.template"]')
                .getAttribute("data-oe-id")
        );
        return rpc("/shop/config/product", {
            product_id: this.plugin.productTemplateID,
            x: x,
            y: y,
        });
    }
...
class SetRibbonAction extends BuilderAction {
    ...
    apply({ editingElement, value }) {
        ...
        this.plugin.productTemplatesRibbons.push({
            templateId: this.plugin.productTemplateID,
            ribbonId: ribbonId,
        });

        const ribbon = this.plugin.ribbonsObject[ribbonId] || {
            id: "",
            name: "",
            bg_color: "",
            text_color: "",
...
class CreateRibbonAction extends BuilderAction {
    apply({ editingElement }) {
        ...
        this.plugin.ribbons.push(ribbon);
        this.plugin.ribbonsObject[ribbonId] = ribbon;
        return this.plugin._setRibbon(editingElement, ribbon);
    }
}
class ModifyRibbonAction extends BuilderAction {
    getValue({ editingElement, params }) {
        ...
        return this.plugin.ribbonsObject[ribbonId][field];
    }
    isApplied({ editingElement, params, value }) {
        ...
        if (!this.plugin.ribbonsObject[ribbonId]) {
            ribbonId = Object.keys(this.plugin.ribbonsObject).find(
                (key) => this.plugin.ribbonsObject[key].id === ribbonId
            );
            editingElement.dataset.ribbonId = ribbonId;
        }
        return this.plugin.ribbonsObject[ribbonId][field] === value;
    }
    apply({ editingElement, params, value }) {
        const isPreviewMode = this.dependencies.history.getIsPreviewing();
        const setting = params.mainParam;
        const ribbonId = parseInt(editingElement.dataset.ribbonId);
        this.plugin.ribbonsObject[ribbonId][setting] = value;

        const ribbon = this.plugin.ribbons.find((ribbon) => ribbon.id == ribbonId);
...
class DeleteRibbonAction extends BuilderAction {
    async apply({ editingElement }) {
        ...
        return this.plugin._deleteRibbon(editingElement);
    }
}
