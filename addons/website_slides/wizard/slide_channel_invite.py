...
class SlideChannelInvite(models.TransientModel):
        ...
        for composer_mixin in self:
            if composer_mixin.template_id.subject:
                if composer_mixin.channel_id.channel_partner_all_ids:
                    ...
                    rec = self.env[self.render_model].new({
    ...
    def _prepare_mail_values(self, slide_channel_partner):
        ...
        subject = self._render_field('subject', slide_channel_partner.ids, set_lang=lang, compute_lang=True)[slide_channel_partner.id]
        ...
        if email_layout_xmlid:
            ...
            template_ctx = {
                ...
                "show_button": True,
                "button_access": {
                    "url": slide_channel_partner.invitation_link,
                    "title": _("Getting Started"),
                },
                "subtitles": [],
                "company": self.env.company,
                "record_name": self.channel_id.name,
                "signature": Markup("<div>-- <br/>%s</div>") % self.channel_id.user_id.signature
                if not is_html_empty(self.channel_id.user_id.signature)
                else False,
            }
