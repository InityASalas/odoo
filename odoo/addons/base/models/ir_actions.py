...
class ServerActionHistoryWizard(models.TransientModel):
    ...
    def _default_action_id(self):
        return self.env['ir.actions.server'].browse(self._context.get('action_id', False))

    action_id = fields.Many2one('ir.actions.server', default=_default_action_id)
    code_diff = fields.Html(compute='_compute_code_diff', sanitize_tags=False)
    revision = fields.Many2one("ir.actions.server.history",
        domain="[('action_id', '=', action_id), ('active', '=', False)]",
        default=lambda self: self.env["ir.actions.server.history"].search(
            [("action_id", "=", self._context.get("action_id", False)), ("active", "=", False)],
            limit=1,
        ),
        required=True,
    )
...
class IrActionsServerHistory(models.Model):
    ...
    _order = 'code_date desc, id desc'
    _log_access = False
    _max_entries_per_action = 100

    action_id = fields.Many2one('ir.actions.server', required=True, ondelete='cascade')
    active = fields.Boolean(string='Current Revision', default=True)
    code = fields.Text()
    code_date = fields.Datetime(default=lambda _: fields.Datetime.now())
...
class IrActionsServer(models.Model):
    ...
    def create(self, vals_list):
        ...
        for action, vals in zip(actions, vals_list):
            if (code := vals.get("code")):
                history_vals.append({"action_id": action.id, "code": code})
        if history_vals:
    ...
    def write(self, vals):
        if (new_code := vals.get('code')):
            ...
            old_code = self.code
            last_history = History.search([('action_id', '=', self.id)], limit=1)
            if last_history:
                last_history.active = False
            else:
                History.create({
                    'action_id': self.id,
                    'code': old_code,
                    'active': False,
                    # in this case we use the record's create_date even though this may not be 100% accurate
                    'code_date': self.create_date.replace(microsecond=0),
                })

            if new_code != old_code:
                History.create({'action_id': self.id, 'code': new_code})
    ...
    def _compute_show_code_history(self):
        ...
        History = self.env['ir.actions.server.history'].with_context(active_test=False)
        for action in self.filtered(lambda a: a.state == 'code'):
            action.show_code_history = History.search_count([('action_id', '=', action.id)]) > 1
