...
class ResPartner(models.Model):
    ...
    def _compute_avatar(self, avatar_field, image_field):
        ...
        partners_to_get_initial_avatar = partners_with_internal_user.filtered(lambda ptr: ptr.user_ids and not ptr.id)
        for partner in partners_to_get_initial_avatar:
            if partner[partner._avatar_name_field]:
                avatar = partner._avatar_generate_svg()
                partner[avatar_field] = avatar
            else:
