...
class TestModuleManifest(BaseCase):
    ...
    def test_default_manifest(self):
        ...
        with self.assertNoLogs('odoo.modules.module', 'WARNING'):
            manifest = dict(Manifest.get_module(self.module_name))
    ...
    def test_change_manifest(self):
        module_name = 'base'
        module = Manifest.get_module(module_name)
        orig_auto_install = module['auto_install']
        with self.assertRaisesRegex(TypeError, r'does not support item assignment'):
            module['auto_install'] = not orig_auto_install
        self.assertIs(Manifest.get_module(module_name), module)

    def test_missing_manifest(self):
        with self.assertLogs('odoo.modules.module', 'DEBUG') as capture:
            manifest = Manifest.get_module(self.module_name)
