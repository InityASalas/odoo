...
class TestGraph(BaseCase):
    ...
    def _test_graph_order(
        ...
        """
        def make_module(name, **kw):
            if name not in dependency:
                return None
            return Manifest(
                path='/dummy/' + name,
                manifest_content={**_DEFAULT_MANIFEST, 'author': 'test', 'license': 'LGPL-3', 'depends': dependency.get(name, [])},
            )

        with (
            patch('odoo.modules.module_graph.ModuleGraph._update_from_database'),
            patch('odoo.modules.module_graph.Manifest.get_module', make_module),
