...
def load_module_graph(
    ...
    with tools.PerfCounter(
        name=f"Loading {module_count} modules",
        log_method=_logger.runbot,
        cr=env.cr,
    ):
        models_updated = set()
        ...
        for index, package in enumerate(graph, 1):
            ...
            with tools.PerfCounter(
                f"Loading module {module_name} ({index}/{module_count})",
                log_method=_logger.debug if not needs_update else _logger.info,
                cr=env.cr,
            ):
