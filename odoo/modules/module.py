...
class Manifest(Mapping[str, typing.Any]):
    ...
    def manifest_cached(self) -> dict:
        ...
        return _load_manifest(self.name, self.manifest_content)
    ...
    def check_manifest_dependencies(self):
    ...
    def __len__(self):
        return sum(1 for _ in self.manifest_cached)

    def __repr__(self):
        return f'Manifest({self.path!r})'

    # limit because this may get called from any module with any input
    @functools.lru_cache(10_000)
    @staticmethod
    def _get_module_from_addons(module: str) -> Manifest | None:
        """Get the module's manifest from a name. Searching only in addons paths."""
        for adp in odoo.addons.__path__:
            if manifest := Manifest._get_module_from_path(opj(adp, module)):
                return manifest
        return None

    @staticmethod
    def get_module(module: str, *, downloaded: bool = False, display_warning: bool = True) -> Manifest | None:
        ...
        if not re.match(r'^\w+$', module):
            # invalid module name
            return None
        if mod := Manifest._get_module_from_addons(module):
            return mod
        if downloaded:
            path = opj(tools.config.addons_data_dir, module)
            if mod := Manifest._get_module_from_path(path):
                return mod
        if display_warning:
            _logger.warning('module %s: manifest not found', module)
        return None

    @staticmethod
    def _get_module_from_path(path: str, env=None) -> Manifest | None:
    ...
    @staticmethod
    def list_modules() -> list[Manifest]:
        """Read all manifests in the addon paths."""
        ...
        for adp in odoo.addons.__path__:
            ...
            for file_name in os.listdir(adp):
                if mod := Manifest._get_module_from_path(opj(adp, file_name)):
...
def get_module_path(module: str, downloaded: bool = False, display_warning: bool = True) -> str | None:
    ...
    """
    # TODO deprecate
    mod = Manifest.get_module(module, downloaded=downloaded, display_warning=display_warning)
...
def get_module_icon(module: str) -> str:
    ...
    manifest = Manifest.get_module(module, display_warning=False)
...
def load_manifest(module: str, mod_path: str | None = None) -> dict:
    ...
    if mod_path:
        mod = Manifest._get_module_from_path(mod_path)
        assert mod.path == mod_path
    else:
        mod = Manifest.get_module(module, downloaded=True)
    if not mod:
...
def get_manifest(module: str, mod_path: str | None = None) -> Mapping[str, typing.Any]:
    ...
    if mod_path:
        ...
        mod = Manifest.get_module(module, downloaded=True, display_warning=False)
...
def load_openerp_module(module_name: str) -> None:
    ...
    try:
        ...
        manifest = Manifest.get_module(module_name)
...
def get_modules() -> list[str]:
    ...
    """
    return [m.name for m in Manifest.list_modules()]


def get_modules_with_version() -> dict[str, str]:
    """Get the module list with the linked version."""
    warnings.warn("Since 19.0, use Manifest.list_modules", DeprecationWarning)
    return {m.name: m.version for m in Manifest.list_modules()}
