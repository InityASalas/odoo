...
regex_order_part_read_group = re.compile(
    r"(?P<term>[\w\.]+(?::(\w+))?)(\s+(?P<direction>desc|asc))?(\s+(?P<nulls>nulls\ first|nulls\ last))?",
)
...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _read_group_groupby_spec(self, alias: str, spec: str, query: Query) -> tuple[BaseModel, str, str]:
        if '.' not in spec:
            return self, alias, spec
        ...
        if field.type != 'many2one':
            raise ValueError(f"Only many2one path is accepted for the {spec!r} groupby spec")
        ...
        return comodel._read_group_groupby_spec(coalias, remaining_spec, query)
    ...
    def _read_group_groupby(self, alias: str, groupby_spec: str, query: Query) -> SQL:
        ...
        accessible for reading.
        """
        model, alias, spec = self._read_group_groupby_spec(alias, groupby_spec, query)
        if spec != groupby_spec:
            return model._read_group_groupby(alias, spec, query)
        ...
        if field.type == 'properties':
            sql_expr = self._read_group_groupby_properties(alias, field, property_name, query)
