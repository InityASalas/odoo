...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None, *, exist_ok: bool = True) -> str:
    ...
    :param exist_ok: check that the file exists (default: True)
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    :raise ValueError: if the file doesn't have one of the supported extensions (`filter_ext`)
    """
    path = file_path(name, filter_ext=filter_ext, env=env, exist_ok=False)
    encoding = None
    if 'b' not in mode:
